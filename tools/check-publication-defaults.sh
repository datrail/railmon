    ...
    *.internal.example.com|*.internal.example.com:[0-9]*)
    ...
    aizawa-metrics.internal)
      [[ "$path" == tools/agent-environment-scanner/scan_agent_environment.py &&
        "$line" == *'called `aizawa-metrics.internal`'* ]] && return 0 ;;
  esac
  return 1
}

parse_scan_output() {
  local output=$1 matches=$2 path= lineno= line= match detector detector_rc failures=
  while :; do
    path=
    ...
    for detector in "$home_pattern" "$internal_pattern"; do
      if printf '%s\n' "$line" | grep -o -i -E "$detector" >>"$matches"; then detector_rc=0; else detector_rc=$?; fi
...
self_test() (
  local fixture index rc output expected path content
  fixture=$(mktemp -d)
  ...
  check_case colon-delimiter tools/agent-environment-scanner/scan_agent_environment.py '/home/node/work:/home/private-user/project' 1 'tools/agent-environment-scanner/scan_agent_environment.py:1:/home/private-user/project' || return
  check_case equals-delimiter tools/agent-environment-scanner/scan_agent_environment.py '/home/node/work=/home/private-user/project' 1 'tools/agent-environment-scanner/scan_agent_environment.py:1:/home/private-user/project' || return
  ...
  check_case bracket-home tests/fixture.py '/home/[private-user]/project' 1 'tests/fixture.py:1:/home/[private-user]/project' || return
  check_case variable-home tests/fixture.py '/home/$USER/project' 1 'tests/fixture.py:1:/home/$USER/project' || return
  check_case unicode-home tests/fixture.py '/home/开发者/project' 1 'tests/fixture.py:1:/home/开发者/project' || return
